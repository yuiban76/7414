...
export function useStoryCatalog(content,enabled){
 ...
 const portraits=['ye','shen','yin','ashina','su'].map(name=>`./assets/companions/${name}.png`);
 const descriptions=['鏢盟青年，追查失蹤師父與自己的遲報責任。','捕快，重新追查自己簽封的舊案。','血河外堂舊人，承認護送涉案貨物，仍須提交舊罪。','北朔斥候，核查自己經手的未登記徵工。','百草醫者，尋回自己簽保證的醫工。'];
 content.npcs.slice(0,5).forEach((npc,i)=>{npc.name=names[i];npc.introduction=descriptions[i];npc.portrait=portraits[i];npc.recruitConditions=i?[{chapterAtLeast:[1,1,2,3,4][i]}]:[];});
