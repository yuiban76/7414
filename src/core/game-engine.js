...
import { CHAPTER_ONE_SCENES } from "./story.js";
...
export function createWorld({ name, startingCountry, ownerCharacterId, seed }) {
  return { id:globalThis.crypto?.randomUUID?.()??`world_${Date.now()}`, name:name.trim()||"風起江湖", schemaVersion:1, contentVersion:CONTENT_VERSION, ownerCharacterId, startingCountry, chapter:1, currentSceneId:"opening", clock:{day:1,segment:"evening"}, party:{playerIds:[ownerCharacterId],npcIds:["npc_001"]}, quests:{quest_chapter_1:"active"}, flags:{}, clues:[], npcStates:{npc_001:{availability:"party"}}, bossStates:{}, cityStates:{}, factionRelations:{}, wantedLevels:{dasheng:0,beishuo:0,nanli:0}, officeState:null, visitedLocations:["location_002"], worldEvents:[], participantHistory:[ownerCharacterId], rng:{seed,counter:0}, updatedAt:new Date().toISOString() };
}

export class GameEngine {
  constructor({ world, character, content }) { this.world=world; this.character=character; this.content=content; }
  get scene() { return CHAPTER_ONE_SCENES[this.world.currentSceneId]; }
  sceneView() { const scene=this.scene; if(!scene)return null; return { ...scene, text:typeof scene.text==="function"?scene.text({countryOpening:COUNTRIES[this.world.startingCountry].opening}):scene.text } }
  choose(choiceId) {
    ...
    if(choice.complete){const chapter=this.content.chapters[0];const mapped=chapter.majorChoices[this.scene.choices.filter(c=>c.complete).findIndex(c=>c.id===choice.id)];chooseMajorPath(this.world,chapter,mapped);this.world.quests.quest_chapter_1="completed";this.world.chapter=2;this.world.currentSceneId="chapter_bridge";}
    else if(!choice.battle)this.world.currentSceneId=choice.next;
    this.world.updatedAt=new Date().toISOString(); return choice;
  }
  finishBattle(choice, result){if(result==="victory"){this.world.currentSceneId=choice.next;if(choice.battle.includes("boss"))this.world.flags.boss_yanzhen_defeated=true;}else{this.world.currentSceneId=choice.battle.includes("boss")?"warehouse":"cart";this.world.flags.last_defeat_scene=choice.battle;}this.world.updatedAt=new Date().toISOString();}
