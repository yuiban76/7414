...
export function restoreBattle(saved){
 ...
 for(const actor of [...saved.party,...saved.enemies])if(actor.buffs!=null&&(!Array.isArray(actor.buffs)||actor.buffs.length>4||actor.buffs.some(buff=>!['damage','posture','defense','speed'].includes(buff.stat)||!Number.isFinite(buff.value)||buff.value<=0||buff.value>.5||!Number.isInteger(buff.remaining)||buff.remaining<1||buff.remaining>5)))throw new Error('戰鬥增益無效');
 if(saved.events!=null&&(!Array.isArray(saved.events)||saved.events.length>200||saved.events.some(e=>!['hit','break','kill','follow','chase','refund','heal','buff'].includes(e.type))))throw new Error('戰鬥回饋無效');
