...
export function makePlayerCombatant(character, skillCatalog) {
  ...
  return {
    ...
    skills: character.equippedSkills.map(id => {const skill=skillCatalog.find(entry=>entry.id===id);const mastery=character.skills?.[id]??{realm:0,completeness:1};if(!skill)return null;const scale=Math.max(.01,mastery.completeness)*(1+mastery.realm*.05);const effects=skill.effects.map(effect=>['heal_pct','battle_buff'].includes(effect.effectId)?{...effect,params:{...effect.params,value:Math.min(.5,Math.round(effect.params.value*scale*1000)/1000)}}:effect);return {...skill,effects,power:{hp:Math.round(skill.power.hp*scale),posture:Math.round(skill.power.posture*scale)},innerCost:Math.max(0,Math.round(skill.innerCost*(1-mastery.realm*.03)))};}).filter(Boolean),
    statuses: [], buffs: [], intent: null, defended: false, guard: null, dodge: 0, vulnerableTurns: 0, observed: 0, consumablesUsed: 0, stableKey: 10
  };
}
...
export function makeNpcCompanion(definition = "陸小川", skillCatalog = [], chapter=1) {
  ...
  const primaryCandidates=skillCatalog.filter(skill=>role.includes('醫療')||role.includes('治療')?skill.effects.some(effect=>effect.effectId==='heal_pct'):role.includes('支援')||role.includes('護衛')?skill.effects.some(effect=>effect.effectId==='battle_buff'):role.includes('身法')?skill.effects.some(effect=>effect.effectId==='dodge'):role.includes('控制')?skill.type==='control':role.includes('防禦')?skill.type==='defense':role.includes('破勢')?skill.tags?.includes('posture_focus'):skill.type==='attack');
  const attacks=skillCatalog.filter(skill=>skill.type==='attack');
  const selected=npc.id==='npc_001'?['流雲劍','踏草步'].map(skillName=>skillCatalog.find(skill=>skill.name===skillName)).filter(Boolean):[primaryCandidates[index%primaryCandidates.length],attacks[(index+7)%attacks.length]].filter(Boolean);
  return { id:npc.id, name:npc.name, side:"party", stats, ...resources, hp:resources.maxHp, posture:resources.maxPosture, inner:resources.maxInner, skills:selected, statuses:[], buffs:[], intent:null, defended:false, guard:null, dodge:0, vulnerableTurns:0, observed:0, consumablesUsed:0, stableKey:10+index };
}
...
export function makeEnemy(definition, skillCatalog, partySize = 2) {
  ...
  return { id:definition.id, name:definition.name, side:"enemy", tier:definition.tier, stats:structuredClone(definition.stats), damageMultiplier:definition.damageMultiplier??(isBoss?BALANCE.bossDamageScale[scaleIndex]:1), maxHp:Math.round(resources.maxHp*hpScale), hp:Math.round(resources.maxHp*hpScale), maxPosture:resources.maxPosture, posture:resources.maxPosture, maxInner:resources.maxInner, inner:resources.maxInner, skills:definition.skillIds.map(id=>skillCatalog.find(s=>s.id===id)).filter(Boolean), statuses:[], buffs:[], phases:structuredClone(definition.phases), triggeredPhases:[], aiProfile:definition.aiProfile, observeInfo:definition.observeInfo, intent:null, defended:false, guard:null, dodge:0, vulnerableTurns:0, observed:0, consumablesUsed:0, stableKey:100 + Number(definition.id.split("_").at(-1)) };
}
...
export class BattleSystem {
  ...
  chooseEnemyAction(actor, includeTarget=true) {
    ...
    let type = actor.aiProfile!=='training' && actor.posture/actor.maxPosture < .25 ? "defend" : "attack";
    const usable = actor.skills.filter(s=>s.innerCost<=actor.inner);
    if (usable.length&&opponents.some(o=>o.vulnerableTurns>0)) type="skill";
    ...
    return { type, skillId:skill?.id??null, targetId:includeTarget ? low?.id??null : null, label:type==="defend"?"防守":type==="skill"?"蓄力／武功":"攻擊" };
  }
  submit(playerAction) {
    ...
    for (const enemy of this.living("enemy")) actions.push({actor:enemy,action:structuredClone(enemy.intent??this.chooseEnemyAction(enemy,true))});
  ...
  resolveSupport(actor,action,skill,actionResult){
    const allies=this.living(actor.side);
    const selected=this.find(action.targetId);
    const targets=skill.target==='self'?[actor]:skill.target==='all_allies'?allies:skill.target==='single_ally'&&selected?.side===actor.side?[selected]:null;
    if(!targets){actionResult.executed=false;this.log.push(`${skill.name}只能施展於自己或同伴。`);return;}
    const cost=Math.max(0,Math.round(skill.innerCost*(1+statusValue(actor,'innerCost'))));
    ...
    if(skill.effects.some(effect=>effect.effectId==='battle_buff'))this.log.push(`${actor.name}施展${skill.name}，提升全隊${({damage:'攻擊',posture:'削架勢',defense:'減傷',speed:'速度'})[skill.effects[0].params.stat]} ${Math.round(skill.effects[0].params.value*100)}%，持續 ${skill.effects[0].params.turns} 回合。`);
  }
  attack(actor, originalTarget, skill, triggered=false) {
    if(this.finished||originalTarget.hp<=0)return;
    this.events??=[];
  ...
  snapshot(){return {events:structuredClone(this.events??[]),scenario:structuredClone(this.scenario??null),round:this.round,party:structuredClone(this.party),enemies:structuredClone(this.enemies),log:[...this.log],finished:this.finished};}
}

function effectiveAgility(actor){return actor.stats.agility*(1+statusValue(actor,"speed")+buffValue(actor,'speed'));}
function buffValue(actor,stat){return Math.max(0,...(actor.buffs??[]).filter(buff=>buff.stat===stat&&buff.remaining>0).map(buff=>buff.value));}
