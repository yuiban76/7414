...
export function sellItem(character,itemId,quantity,itemCatalog,location){const item=itemCatalog.find(row=>row.id===itemId);if(!item)throw new Error("商品不存在");const total=priceAt(item,location,{selling:true})*quantity;removeItem(character,itemId,quantity);character.moneyWen=(character.moneyWen??0)+total;return total;}
