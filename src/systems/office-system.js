...
export function availableOfficeTasks(world,location){if(!world.officeState||!location?.facilities?.includes("government"))return[];return OFFICE_TASKS.filter(task=>task.minRank<=world.officeState.rankIndex&&!world.officeState.completedTasks.includes(`${task.id}:${world.clock.day}:${location.id}`));}
