...
for(const [id,scene] of Object.entries(BATTLES).filter(([id])=>!process.env.ZHAOYE_SCENE||id===process.env.ZHAOYE_SCENE))for(const size of [1,2,3,4]){
 ...
 for(const identity of IDENTITIES)for(const startSkillName of identity.skills){let wins=0;
  for(let seed=0;seed<40;seed++){
   ...
   const enemy=makeEnemy({...def,name:scene.name},skills,size);scaleStoryEncounter(enemy,{currentSceneId:id,zhaoye:createZhaoyeState()},size);const battle=new BattleSystem({party,enemies:[enemy],scenario:createStoryScenario(id),rng:new SeededRng(`zhaoye:${id}:${size}:${startSkillName}:${seed}`)});
