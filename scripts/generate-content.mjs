...
import { adaptSkillArchetype } from "../src/config/skill-archetypes.js";
...
const npcs = npcRows.map((r, i) => ({ id: slug("npc", i + 1), name: r[1], region: r[2], role: r[3], introduction: r[4].replaceAll("流影步","流影鎖步"), growthProfile: r[3], recruitConditions: i === 0 ? [] : [{ chapterAtLeast: Math.min(8, Math.max(1, Math.ceil((i + 1) / 3))) }], relationshipState: i === 0 ? "信任" : "陌生", availability: i === 0 ? "party" : "locked", aiProfile: r[3].includes("護衛") ? "protective" : r[3].includes("控制") ? "cautious" : "aggressive", storyFlags: [] }));

const enemyRows = tableAfter("### 14.3 V1 名單");
const enemies = enemyRows.map((r, i) => { const chapter = num(r[1]); const tier = r[4] === "精英" ? "elite" : r[4] === "Boss" ? "boss" : "major_boss"; const base = 16 + chapter * 5 + (tier === "major_boss" ? 12 : tier === "boss" ? 6 : 0); const desired=r[5].includes("破勢")?skill=>skill.tags.includes("posture_focus"):r[5].includes("防禦")?skill=>skill.type==="defense":skill=>skill.type==="attack"&&!skill.tags.includes("posture_focus"); return { id: slug("enemy", i + 1), name: r[2], chapter, faction: r[3], tier, realm: Math.min(5, Math.floor((chapter - 1) / 1.5)), stats: { strength: base + 3, constitution: base + 5, agility: base, comprehension: base - 2, willpower: base + 1 }, skillIds: skills.filter(desired).slice(0, tier === "elite" ? 3 : 6).map(s => s.id), phases: tier === "major_boss" ? [{ trigger: { type: "hp_below", value: .6 }, effect: "phase_two" }, { trigger: { type: "hp_below", value: .25 }, effect: "desperate" }] : [], aiProfile: r[5].includes("護衛") ? "protective" : r[5].includes("控制") ? "cunning" : "aggressive", observeInfo: [r[5]], rewards: [], storyFlags: [], scalingProfile: `chapter${chapter}_${tier}` }; });
