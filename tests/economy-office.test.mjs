...
import { equipmentRarityLabel, equipmentStatSummary } from "../src/ui/shop-labels.js";
...
test("shop equipment rarity and stat bonuses render in Traditional Chinese",()=>{assert.equal(equipmentRarityLabel("common"),"普通");assert.equal(equipmentRarityLabel("fine"),"精良");assert.equal(equipmentRarityLabel("famed"),"名品");assert.equal(equipmentRarityLabel("legendary"),"傳奇");assert.equal(equipmentStatSummary({damage:8,postureDamage:5,damageReduction:.021}),"傷害 +8　架勢傷害 +5　傷害減免 +2.1%");for(const item of equipment){assert.doesNotMatch(equipmentRarityLabel(item.rarity),/[A-Za-z]/);assert.doesNotMatch(equipmentStatSummary(item.stats),/[A-Za-z]/);}});
